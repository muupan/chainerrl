"""Mimic the setting used in baselines

- Adam(1e-4, eps=1e-4) (loss is mean, not sum)
  - https://github.com/openai/baselines/blob/master/baselines/deepq/experiments/atari/train.py#L132
- Gradient clipping =10
  - https://github.com/openai/baselines/blob/master/baselines/deepq/experiments/atari/train.py#L134
- target_update_interval=40000
  - https://github.com/openai/baselines/blob/master/baselines/deepq/experiments/atari/train.py#L42
...
from wrap_deepmind import wrap_deepmind
...
def dqn_phi(obs):
    assert obs._frames[0].dtype == np.uint8
    obs = np.stack(obs._frames).astype(
        np.float32, copy=False)
    assert obs.shape == (4, 84, 84, 1)
    return obs.reshape((4, 84, 84)) / 255.0
...
def main():
    ...
    opt = optimizers.Adam(1e-4, eps=1e-4)
    ...
    if args.demo:
        ...
        epsilon_decay_hook = experiments.PiecewiseLinearInterpolationHook(
            1.0, [(10 ** 6, 0.1), (10 ** 7, 0.01)], setter=epsilon_setter)
